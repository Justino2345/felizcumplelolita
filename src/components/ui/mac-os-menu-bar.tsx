  ...
  return (
    <div
      ...
      style={{
        ...
        `,
        minWidth: '216px',
        animation: 'menuFadeIn 0.14s cubic-bezier(0.23, 1, 0.32, 1) forwards'
      }}
    >
        ...
        {items.map((item, index) => {
          if (item.type === 'separator') {
            return (
              <div
                key={index}
                className="mx-2.5 my-1 h-px bg-white/10"
              />
            );
          }
          ...
          return (
            <div
              key={index}
              className="mx-1.5 flex cursor-pointer items-center justify-between rounded-md px-3 py-[5px] text-[13px] tracking-[-0.01em] text-white/90 transition-colors duration-100 hover:bg-white/10"
              onClick={() => {
                if (item.action) {
            ...
            >
              ...
              {item.shortcut && (
                <span className="tnum ml-4 text-[12px] text-white/45">
                  {item.shortcut}
                </span>
              )}
            </div>
...
const MacOSMenuBar: React.FC<MacOSMenuBarProps> = ({
  ...
  return (
    <div style={{ position: 'relative' }}>
      <div
        className={`hairline relative flex overflow-hidden text-white ${className}`}
        style={{
          height: '32px',
          borderRadius: '8px',
      ...
      >
        ...
        />
        {/* scrim oscuro tenue: da contraste al texto blanco sin tapar el vidrio */}
        <div
          className="absolute inset-0 z-10 rounded-inherit"
          style={{ background: 'rgba(24, 24, 27, 0.28)' }}
        />
        <div
          className="absolute inset-0 z-10 rounded-inherit"
        ...
        <div className="relative z-30 flex w-full items-center justify-between gap-2 h-full px-2.5 sm:px-4">
          ...
          <div className="flex min-w-0 flex-1 items-center gap-2 sm:gap-4">
            ...
            {/* Current App Name */}
            <span className="shrink truncate text-[13px] font-medium tracking-[-0.01em] text-white/95 max-w-[30vw] sm:max-w-none">
              {appName}
            </span>

            {/* Menu Items */}
            <div className="flex min-w-0 items-center gap-2.5 overflow-x-auto sm:gap-6 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              {menus.map((menu) => (
                <span
                  key={menu.label}
                  ref={(el) => { menuRefs.current[menu.label] = el; }}
                  className="shrink-0 cursor-pointer select-none text-[13px] tracking-[-0.01em] text-white/85 transition-opacity duration-150 hover:opacity-70"
          ...
          <div className="flex shrink-0 items-center gap-2 sm:gap-4">
            {/* Battery */}
            <div className="hidden cursor-pointer hover:opacity-80 transition-opacity duration-150 sm:block">
            ...
            {/* Clock */}
            <span
              className="tnum shrink-0 cursor-pointer select-none text-[13px] tracking-[-0.01em] text-white/85 transition-opacity duration-150 hover:opacity-70"
            >
              {currentTime}
            </span>
          </div>
...
export type { MenuConfig, MenuItemOption, MacOSMenuBarProps };
