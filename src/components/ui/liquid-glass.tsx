...
export const GlassEffect: React.FC<GlassEffectProps> = ({
  ...
  const content = (
    ...
    >
      {/* Glass Layers */}
      <div
        className="glass-distortion-fx absolute inset-0 z-0 overflow-hidden rounded-inherit rounded-3xl"
        style={{
...
export const GlassDock: React.FC<{ icons: DockIcon[]; href?: string }> = ({
  ...
  <GlassEffect href={href} className="rounded-3xl p-1.5 hover:p-2">
    <div className="flex items-center justify-center gap-1.5 rounded-3xl px-1 py-0">
      {icons.map((icon, index) => (
        <img
          key={index}
          src={icon.src}
          alt={icon.alt}
          className="w-11 h-11 rounded-[22%] transition-all duration-700 hover:scale-[1.06] cursor-pointer"
          style={{
          ...
          onError={(e) => {
            if (icon.fallbackSrc && e.currentTarget.src !== icon.fallbackSrc) {
              e.currentTarget.src = icon.fallbackSrc;
            }
          }}
        />
      ))}
